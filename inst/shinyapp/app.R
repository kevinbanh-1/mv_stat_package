...
server <- function(input, output,session) {
    ...
    output$ggplot <- renderPlot({
        ...
        suppressWarnings({
        if(type == "Lines"){
            g <- ggplot(data(), aes(x = dat[[input$xvar]], y = dat[[input$yvar]])) + geom_line() + 
                xlab(input$xvar) + ylab(input$yvar) + ggtitle(paste0(input$yvar, " vs. ", input$xvar)) + 
                theme(plot.title = element_text(hjust = .5))
        }
        
        if(type == "Points"){
            g <- ggplot(data(), aes(x = dat[[input$xvar]], y = dat[[input$yvar]])) + geom_point() + 
                xlab(input$xvar) + ylab(input$yvar) + ggtitle(paste0(input$yvar, " vs. ", input$xvar)) + 
                theme(plot.title = element_text(hjust = .5))
        }
        
        if(type == "Smooth"){
            g <- ggplot(data(), aes(x = dat[[input$xvar]], y = dat[[input$yvar]])) + geom_smooth() + 
                xlab(input$xvar) + ylab(input$yvar) + ggtitle(paste0(input$yvar, " vs. ", input$xvar)) + 
                theme(plot.title = element_text(hjust = .5))
        }
        
        if(type == "Jitter"){
            g <- ggplot(data(), aes(x = dat[[input$xvar]], y = dat[[input$yvar]])) + geom_jitter() + 
                xlab(input$xvar) + ylab(input$yvar) + ggtitle(paste0(input$yvar, " vs. ", input$xvar)) + 
                theme(plot.title = element_text(hjust = .5))
        }
        
        if(type == "Quantile"){
            g <- ggplot(data(), aes(x = dat[[input$xvar]], y = dat[[input$yvar]])) + geom_quantile() + 
                xlab(input$xvar) + ylab(input$yvar) + ggtitle(paste0(input$yvar, " vs. ", input$xvar)) + 
                theme(plot.title = element_text(hjust = .5))
        }
    ...
    output$rotatedPlot <- renderPlot({                                                                       
        ...
        suppressWarnings({
            if(type == "Lines"){
                g.transf <- ggplot(trans.val, aes(x = transformed.x, y = transformed.y)) + geom_line() + 
                    xlab(input$xvar) + ylab(input$yvar) + ggtitle(paste0(input$yvar, " vs. ", input$xvar)) + 
                    theme(plot.title = element_text(hjust = .5))
            }
            
            if(type == "Points"){
                g.transf <- ggplot(trans.val, aes(x = transformed.x, y = transformed.y)) + geom_point() + 
                    xlab(input$xvar) + ylab(input$yvar) + ggtitle(paste0(input$yvar, " vs. ", input$xvar)) + 
                    theme(plot.title = element_text(hjust = .5))
            }
            
            if(type == "Smooth"){
                g.transf <- ggplot(trans.val, aes(x = transformed.x, y = transformed.y)) + geom_smooth() + 
                    xlab(input$xvar) + ylab(input$yvar) + ggtitle(paste0(input$yvar, " vs. ", input$xvar)) + 
                    theme(plot.title = element_text(hjust = .5))
            }
            
            if(type == "Jitter"){
                g.transf <- ggplot(trans.val, aes(x = transformed.x, y = transformed.y)) + geom_jitter() + 
                    xlab(input$xvar) + ylab(input$yvar) + ggtitle(paste0(input$yvar, " vs. ", input$xvar)) + 
                    theme(plot.title = element_text(hjust = .5))
            }
            
            if(type == "Quantile"){
                g.transf <- ggplot(trans.val, aes(x = transformed.x, y = transformed.y)) + geom_quantile() + 
                    xlab(input$xvar) + ylab(input$yvar) + ggtitle(paste0(input$yvar, " vs. ", input$xvar)) + 
                    theme(plot.title = element_text(hjust = .5))
            }
        })
        ...
        suppressWarnings({
            print(g.transf, vp=viewport(angle=(2*input$theta * 180)/3.14,  width = unit(.75, "npc"), height = unit(.75, "npc")))
        })
    })
